...
3. A lot of the DNS queries have a destination of 8.8.8.8. However, a subset have a destination for 18.217.1.57, which stands out like a **red flag**.
4. We can apply the filter `dns && ip.dst==18.217.1.57` to only see DNS requests to this IP address. 
5. If we extract each subdomain (everything before .reddshrimpandherring.com) and append them in the order they appear (sort by No.), we get: `aGNpe3NvX2FfNWhyMW1wX2ZyMTNkX3RoMXNfcjFjMz99`.
6. Since Shrimp's restaurant is named Base-64 (all his food would be encoded in base64), we can decode the string as base64 in CyberChef to obtain the flag.

**Flag**: `hci{so_a_5hr1mp_fr13d_th1s_r1c3?}`
